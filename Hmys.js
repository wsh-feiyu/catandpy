...
let host = 'http://dy.stxbed.com', play_domain = '', encrypt_domain = encrypt_domain_def, siteKey = '', siteType = 0;
let headers = {
...
async function init(cfg) {
    ...
    try {
        ...
        // 空 ext 时回退到默认 host，保证调试/模拟器无参数也能出数据
        host = (ext.host || host).replace(/\/$/, '');
        const appId = ext.appid || ext.app_id || "qihang";
        const channel = ext.UMENG_CHANNEL || "zhuan";
        Object.assign(headers, {
            appid: appId, 'Version-Code': ext.versionCode || "11000", 'Channel': channel,
            'Device-Id': ext.deviceid || await local.get('Hmys_deviceid', `${appId}_${channel}`) || generateDeviceId()
        });
        if (!ext.deviceid) await local.set('Hmys_deviceid', `${appId}_${channel}`, headers['Device-Id']);
